...
Main issue when using BFS for a non-predictive top-down parser:
- BFS doesn't account for the order of the rules specified in the CFG.
- Due to this, it explores all possible paths, which can be a big waste of time.
- In the worst case, it can explore all possible paths before finding a match.
Especially if we want to implement LL(0) or LL(1), this isn't feasible at all.

Example configuration highlighting how slow it is using the current CFG:
- CFG: E -> E + T | T, T -> n
- Input: n+n+n+n++n
- The parser will explore all possible paths, including those that are invalid.
- This can lead to a large number of iterations before finding a match.

Reason for this inefficiency:
- BFS explores all possible paths in a breadth-first manner.
- It doesn't take into account the structure of the CFG or the order of rules.
*/